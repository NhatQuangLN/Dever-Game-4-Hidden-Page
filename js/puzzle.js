(function(window, document, undefined){
	var 	image = new Image(),
			...
			COLS = 3,
			tiles = [],
			SPRITE_SHEET = "url('img/puzzle.png')",
			mouseX,
			mouseY,
			offsetX,
			offsetY,
	...
	function mouseDown(e){
		...
		var c = complete();
		if(c == ROWS * COLS){
            abc.setAttribute('style','display:inline-block');
			abc.setAttribute("style", 'cursor: pointer;');
		}
	}
	...
	function init(){
		...
		statusP = document.createElement("marquee");
		abc = document.createElement('img');
		abc.setAttribute("width", '50px');
		abc.setAttribute("height", 'auto');
        abc.setAttribute("src", 'img/key.png');
		abc.onclick=function(){
			alert('-------------You Win!-------------\n' + 'Capture this and comment to get the prized!\n'+"Anwser Today: Develop Of Innovation!");
		}
		statusP.appendChild(abc);
		// statusP.setAttribute('direction','left');
		abc.setAttribute('style','display:none;');
		div.parentNode.appendChild(statusP);

	}
	image.onload = init;
	image.src = "img/puzzle.png";
